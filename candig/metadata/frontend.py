...
class ServerStatus(object):
    ...
    def getServerVersion(self):
        ...
        """
        return candig.metadata.__version__
...
def reset():
    ...
    """
    app.config.clear()
    configStr = 'candig.metadata.serverconfig:FlaskDefaultConfig'
...
def configure(configFile=None, baseConfig="ProductionConfig",
    ...
    configStr = 'candig.metadata.serverconfig:{0}'.format(baseConfig)
    app.config.from_object(configStr)
...
def federation(endpoint, request, return_mimetype, request_type='POST'):
    ...
    responseObject['status'] = {
        ...
        'Known peers': 1, #len(app.serverStatus.getPeers()) + 1,
